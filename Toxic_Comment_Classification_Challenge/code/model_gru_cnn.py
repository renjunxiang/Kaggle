...
def gru_cnn(input_dim, input_length=100, output_dim=100, label_n=6):
    '''
...
model_gru_cnn = gru_cnn(input_dim=input_dim, input_length=100, output_dim=100, label_n=6)
...
train_train_x=np.load('./data_seq/train_train_x.npy')
train_test_x=np.load('./data_seq/train_test_x.npy')
train_train_y=np.load('./data_seq/train_train_y.npy')
train_test_y=np.load('./data_seq/train_test_y.npy')


model_gru_cnn.fit(x=train_train_x, y=train_train_y,
               validation_data=[train_test_x, train_test_y],
               batch_size=batch_size, epochs=epochs, verbose=1)
model_gru_cnn.save('./models/gru_cnn_%d_%d_%d.h5' % (samples, batch_size, epochs))

test_data_new=np.load('./data_seq/test_data_new.npy')
test_label = model_gru_cnn.predict(test_data_new)
...
test_result.to_csv('./result/gru_cnn_%d_%d_%d.csv' % (samples, batch_size, epochs), index=False)
